...
import { adminMetroStations } from '@/api/admin'
...
import type { MetroStationDto } from '@/types'

const { t } = useI18n()
const { items, loading, create, update, remove } = useAdminCrud<MetroStationDto>(adminMetroStations)

const showForm = ref(false)
const showConfirm = ref(false)
const editing = ref<MetroStationDto | null>(null)
...
const columns = [
  ...
  { key: 'lineColor', label: t('admin.fields.lineColor') },
...
function openCreate() {
  editing.value = null
  formModel.value = { name: '', slug: '', lineColor: '#0066cc', cityId: 1 }
  showForm.value = true
}

function openEdit(item: AdminTableRow) {
  const station = item as unknown as MetroStationDto
...
<template>
  <div>
    <AdminPageHeader :title="t('admin.metroStations')">
      <template #actions>
