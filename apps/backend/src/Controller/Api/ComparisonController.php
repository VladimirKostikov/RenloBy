...
class ComparisonController extends AbstractController
{
    ...
    #[Route('', name: 'api_comparisons_index', methods: ['GET'])]
    public function index(Request $request, #[CurrentUser] ?User $user): JsonResponse
    {
        ...
        return $this->collectionRequestResolver->json([
            'items' => $this->comparisonService->listWithListings($context->owner),
        ], $context);
    }

    #[Route('', name: 'api_comparisons_create', methods: ['POST'])]
    public function create(Request $request, #[CurrentUser] ?User $user): JsonResponse
    {
        $context = $this->collectionRequestResolver->resolve($request, $user);
        $dto = $this->requestMapper->mapCreateComparison($this->requestMapper->decodeJson($request));
        $this->assertValid($dto);

        $comparison = $this->comparisonService->add($context->owner, $dto);

        return $this->collectionRequestResolver->json($comparison, $context, Response::HTTP_CREATED);
    }

    #[Route('/toggle', name: 'api_comparisons_toggle', methods: ['POST'])]
    public function toggle(Request $request, #[CurrentUser] ?User $user): JsonResponse
    {
        ...
        return $this->collectionRequestResolver->json(
            $this->comparisonService->toggle($context->owner, $dto->listingId),
            $context,
        );
    }

    #[Route('/{id}', name: 'api_comparisons_delete', methods: ['DELETE'], requirements: ['id' => '\d+'])]
    public function delete(Request $request, int $id, #[CurrentUser] ?User $user): Response
    {
        $context = $this->collectionRequestResolver->resolve($request, $user);
        $this->comparisonService->remove($context->owner, $id);
